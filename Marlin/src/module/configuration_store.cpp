...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    EEPROM_WRITE(repeat_markers_count);
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      if(repeat_markers_count != 0)
      {
        repeat_markers_count = 0;
      }
      EEPROM_READ(repeat_markers_count);
...
void MarlinSettings::reset() {
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    CONFIG_ECHO_HEADING("repeat markers count:");
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(PSTR("  M808 L"), repeat_markers_count);
  }
