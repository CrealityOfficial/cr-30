...
void CardReader::fileHasFinished() {
  ...
  else if (repeat_markers_count > 0)
  {
    // Repeat markers count file after closing procedure
    repeat_markers_count--;
    settings.save();
    card.openFileRead(card.filename);
    recovery.info.recovery_repeat_markers_flag = false;
    startFileprint();
  ...
  void CardReader::removeJobRecoveryFile()
  {
    if (jobRecoverFileExists())
    {
      ...
      recovery.info.recovery_repeat_markers_flag = true;
